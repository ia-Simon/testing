...
function makeIntrospectionQueryDocument(introspectionSelection) {
    return {
...
function executeIntrospection(schema, introspectionSelection) {
    const { data } = (0, graphql_1.executeSync)({
        schema,
    ...
    return opentelemetry_1.tracer.startActiveSpan(opentelemetry_1.OpenTelemetrySpanNames.EXECUTE, async (span) => {
        try {
            ...
            const result = await opentelemetry_1.tracer.startActiveSpan(opentelemetry_1.OpenTelemetrySpanNames.POST_PROCESSING, async (span) => {
                let data;
                try {
                    const operation = (0, federation_internals_1.operationFromDocument)(apiSchema, {
                        kind: graphql_1.Kind.DOCUMENT,
                        definitions: [
        ...
        try {
            ...
            if (!fetch.requires) {
                ...
                for (const entity of entities) {
                    (0, deepMerge_1.deepMerge)(entity, withFetchRewrites(dataReceivedFromService, fetch.outputRewrites));
                }
            }
            else {
                ...
                entities.forEach((entity, index) => {
                    const representation = executeSelectionSet(context.supergraphSchema, entity, requires, fetch.inputRewrites);
                ...
                for (let i = 0; i < entities.length; i++) {
                    (0, deepMerge_1.deepMerge)(entities[representationToEntity[i]], withFetchRewrites(receivedEntities[i], filterEntityRewrites(representations[i], fetch.outputRewrites)));
                }
            }
        }
        catch (err) {
...
function applyOrMapRecursive(value, fct) {
    if (Array.isArray(value)) {
        const res = value.map((elt) => applyOrMapRecursive(elt, fct)).filter(federation_internals_1.isDefined);
        return res.length === 0 ? undefined : res;
    }
    return fct(value);
}
function withFetchRewrites(fetchResult, rewrites) {
    ...
    for (const rewrite of rewrites) {
        ...
        while (obj && i < rewrite.path.length - 1) {
            ...
            if (p.startsWith('... on ')) {
                ...
                obj = applyOrMapRecursive(obj, (elt) => elt[graphql_1.TypeNameMetaFieldDef.name] === typename ? elt : undefined);
            }
            else {
...
function filterEntityRewrites(entity, rewrites) {
    if (!rewrites) {
        return undefined;
    }
    const typename = entity[graphql_1.TypeNameMetaFieldDef.name];
    const typenameAsFragment = `... on ${typename}`;
    return rewrites.map((r) => r.path[0] === typenameAsFragment ? { ...r, path: r.path.slice(1) } : undefined).filter(federation_internals_1.isDefined);
}
function updateRewrites(rewrites, pathElement) {
    if (!rewrites) {
        return undefined;
    }
    let completeRewrite = undefined;
...
function executeSelectionSet(schema, source, selections, activeRewrites) {
    if (source === null) {
        return null;
    }
    ...
    for (const selection of selections) {
        switch (selection.kind) {
            case graphql_1.Kind.FIELD:
                ...
                if ((updatedRewrites === null || updatedRewrites === void 0 ? void 0 : updatedRewrites.completeRewrite) !== undefined) {
                    result[responseName] = updatedRewrites.completeRewrite;
                    continue;
                }
                if (Array.isArray(source[responseName])) {
                    result[responseName] = source[responseName].map((value) => selections
                        ? executeSelectionSet(schema, value, selections, updatedRewrites === null || updatedRewrites === void 0 ? void 0 : updatedRewrites.updated)
                        : value);
                }
                else if (selections) {
                    result[responseName] = executeSelectionSet(schema, source[responseName], selections, updatedRewrites === null || updatedRewrites === void 0 ? void 0 : updatedRewrites.updated);
                }
                else {
            ...
            case graphql_1.Kind.INLINE_FRAGMENT:
                ...
                if (doesTypeConditionMatch(schema, selection.typeCondition, typename)) {
                    ...
                    (0, deepMerge_1.deepMerge)(result, executeSelectionSet(schema, source, selection.selections, updatedRewrites === null || updatedRewrites === void 0 ? void 0 : updatedRewrites.updated));
                }
                break;
        }
    }
